    ...
    return [
      (
        <li key="0"><a href="#" onClick={() => this.updateFormStatus('register')}>
          {getTranslation(this.props.lang, 'Registrarse')}
        </a></li>
      ),
      (
        <li key="1"><a href="#" onClick={() => this.updateFormStatus('login')}>
          {getTranslation(this.props.lang, 'Iniciar Sesión')}
        </a></li>
      )
    ];
  }
        ...
        <div className={'homeNav' + (this.state.isHandlerActive ? ' active' : '')} >
          <ul>
            <li className="moreOption">
              <Link to="/search?type=sale">{getTranslation(this.props.lang, 'Comprar')}</Link>
            </li>
            <li className="moreOption">
              <Link to="/search?type=rent">{getTranslation(this.props.lang, 'Alquilar')}</Link>
            </li>
            {/* <li className="moreOption">
              <Link to="/agent/search">{getTranslation(this.props.lang, 'Renovación')}</Link>
            </li>
            <li className="moreOption">
              <Link to="/agent/search">{getTranslation(this.props.lang, 'Comercial')}</Link>
            </li>
            <li className="moreOption">
              <Link to="/projects">{getTranslation(this.props.lang, 'Proyecto')}</Link>
            </li>
            <li className="moreOption">
              <Link to="/agent/search">{getTranslation(this.props.lang, 'Encontrar agente')}</Link>
            </li> */}
            {this.loginField()}
{/*             <li>
              <Link to="/newproperty/sell"><div className="btn btn-green">
                {getTranslation(this.props.lang, 'Listar Propiedades')}
              </div></Link>
            </li> */}
          </ul>
        </div>
